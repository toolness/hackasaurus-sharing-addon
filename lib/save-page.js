...
exports.saveCurrentPage = function saveCurrentPage(formData, cb) {
  ...
  internalSave({
    ...
    success: function() {
      ...
      formData.append('index_support_files', addedFileCount);

      for (var i = 0; i < elem.files.length; i++) {
        formData.append(fnames[i], elem.files[i]);
        console.log(fnames[i], elem.files[i].name);
        var j = i-1;
        if (j in fdirs) {
          formData.append(fnames[i] + '_dir', fdirs[j]);
          console.log(fnames[i] + '_dir', fdirs[j]);
        }
      }
