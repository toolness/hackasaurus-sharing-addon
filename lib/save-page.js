...
exports.saveCurrentPage = function saveCurrentPage(postURL, cb) {
    ...
    success: function() {
      ...
      if (fileFolder.exists())
        require('file').list(fileFolder.path).forEach(function(name) {
          ...
          f.append(name);
          files.push(f.path);
          fnames.push('file-' + addedFileCount);
          addedFileCount++;
        });
      ...
      f.append('url', url);
      f.append('addedFileCount', addedFileCount);

      for (var i = 0; i < elem.files.length; i++)
        f.append(fnames[i], elem.files[i]);
